...
/* 内存告警阈值（字节） */
#define MY_MEM_CRITICAL_THRESHOLD       (1024U)   /* 严重阈值 */
#define MY_MEM_WARN_THRESHOLD           (2048U)   /* 警告阈值 */
#define MY_MEM_MIN_THRESHOLD            (512U)    /* 历史最小值告警 */
 ...
 *********************************************************************/
void my_mem_get_stats(size_t *free_size, size_t *min_free_size, uint8_t *usage_percent)
{
    if (free_size != NULL)
    {
